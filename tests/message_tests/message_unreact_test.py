...
'''
--------------------------------------------------------
    Test for Messages Unreact on channels
--------------------------------------------------------
'''
...
# Input error for Invalid react_id
# Input error when the authorised user never reacted to the message 

# When message ID doesn't exist - Channel
def test_invalid_message_id(setup1):
    with pytest.raises(InputError):
        assert message_unreact_v1(setup1[2], 1729, 1)
        
# When user in not a part of the dm/channel where message id is in - Channel
def test_unauthorised_message_id1(setup1):
    with pytest.raises(InputError):
        assert message_unreact_v1(setup1[4], setup1[1], 1)
        
# When user in not a part of the dm/channel where message id is in - DM
def test_unauthorised_message_id2(setup2):
    with pytest.raises(InputError):
        assert message_unreact_v1(setup2[4], setup2[1], 1)
        
# When user does not exist/ Invalid Token
def test_invalid_token(setup1):
    invalid_token = -1
    with pytest.raises(AccessError):
        assert message_unreact_v1(invalid_token, setup1[0], 1)
        
# When user uses incorrect react id - Channel
def test_invalid_unreact_id1(setup1):
    with pytest.raises(InputError):
        assert message_unreact_v1(setup1[1], setup1[0], 10)
        
# When user uses incorrect react id - DM
def test_invalid_unreact_id2(setup2):
    with pytest.raises(InputError):
        assert message_unreact_v1(setup2[1], setup2[0], 10)
        

# Test successful react on channel messages
def test_successful_unreact1(setup1):
    message_react_v1(setup1[2], setup1[0], 1)
    message_unreact_v1(setup1[2], setup1[0], 1)
    msgs = channel_messages_v2(setup1[2], setup1[5], 0).get('messages')
    curr_msg_reacts = msgs[0].get('reacts')
    assert len(curr_msg_reacts[0]['u_ids']) == 0
        
# Test successful react on dm messages
def test_successful_unreact2(setup2):
    message_react_v1(setup2[2], setup2[0], 1)
    message_unreact_v1(setup2[2], setup2[0], 1)
    msgs = dm_messages_v1(setup2[2], setup2[5], 0).get('messages')
    curr_msg_reacts = msgs[0].get('reacts')
    assert len(curr_msg_reacts[0]['u_ids']) == 0
    
# When user tries to unreact with a react id which they never reacted with - Channel
def test_never_reacted1(setup1):
    with pytest.raises(InputError):
        message_unreact_v1(setup1[2], setup1[0], 1)

        
# When user tries to unreact with a react id which they never reacted with - Dm
def test_never_reacted2(setup2):
    with pytest.raises(InputError):
        message_unreact_v1(setup2[2], setup2[0], 1)
        
# Note for future tests - when we have more react ids we can potentially do more tests
# with unreacting with never used react ids
